...
class FingerDMControlWrapper(DMControlWrapper):
    ...
    def modify_mjcf_model(self, mjcf_model):
        ...
        base = proximal_geom.density if proximal_geom.density is not None else 1000.0
        desired_density = float(np.asarray(self.variation_space["agent"]["proximal_density"].value).reshape(-1)[0])
        ...
        base = fingertip_geom.density if fingertip_geom.density is not None else 1000.0
        desired_density = float(np.asarray(self.variation_space["agent"]["fingertip_density"].value).reshape(-1)[0])
        if not np.allclose(base, desired_density):
            mass_changed = True
        ...
        if shape_id == 0:
            desired_type = "box"
            desired_size = np.array([_HARD_TARGET_SIZE, _HARD_TARGET_SIZE, _HARD_TARGET_SIZE], dtype=np.float32)
        else:
            desired_type = "sphere"
