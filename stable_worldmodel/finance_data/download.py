...
def _download_and_save(tickers, start_time, end_time, hdf_path, ticker_batch_size=1000, timestamps_batch_size=1):
    ...
    for ticker_batch, range_start, range_end in tqdm(batches, desc="Downloading data"):
        ...
        bars_req = StockBarsRequest(symbol_or_symbols=ticker_batch, start=range_start, end=range_end, timeframe=TimeFrame.Minute)
        ...
        if bars_values is not None and not bars_values.empty:
            logger.info("Starting trade values processing")
            ...
            logger.warning("SKIPPING batch due to no data returned from API")
...
def load_market_data(tickers, start_time, end_time, freq):
    ...
    if not os.path.exists(hdf_path):
        ...
        if needs_new_tickers:
            ...
            logger.success(f"Downloaded data for new tickers")
        ...
        if needs_earlier_data:
            ...
            logger.success(f"Downloaded earlier data")
        ...
        if needs_later_data:
            ...
            logger.success(f"Downloaded later data")
