...
def get_sector_tickers(sector_config, pad_tickers_ratio=1):
    ...
    df1 = pd.read_csv(os.path.join(current_dir, "nyse.csv"))
    df2 = pd.read_csv(os.path.join(current_dir, "nasdaq.csv"))
    ...
    all_sector_tickers = df.groupby("Sector")["Symbol"].apply(lambda x: sorted(x.unique())).to_dict()
    ...
    if not sector_config:
        ...
        selected_sector_tickers = dict(all_sector_tickers.items())
        return selected_sectors, selected_sector_tickers

    for sector in sector_config.keys():
        if sector not in all_sector_tickers:
            raise ValueError(
                f"Sector '{sector}' not found in data. Available sectors: {list(all_sector_tickers.keys())}"
            )
    ...
    for sector, stocks_per_patch in sector_config.items():
        ...
        if available < adjusted_stocks:
            logger.warning(
                f"Sector '{sector}' has only {available} stocks, requested {adjusted_stocks} (including buffer)"
            )
...
def build_stock_data(start_time, end_time, processing_methods, sector_config, freq):
    ...
    logger.info(
        "Post-filtering: "
        + ", ".join(f"{s}: {len(selected_sector_tickers[s])}/{sector_config[s]}" for s in selected_sectors)
    )
